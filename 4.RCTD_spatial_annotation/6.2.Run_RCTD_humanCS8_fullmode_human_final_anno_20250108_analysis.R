...
setwd("D:/xueying-work/afterPHD/Liu-lab/project1-embryo benchmarking/2024April/manuscript/final code/20250108_runRCTD_with_updatedanno")
outputDir = getwd()
...
#############################################################################
# Define the list of cell types to highlight
cell_types <- c("Epi_3", "Emergent mesoderm", "DE", "YS.Endoderm_1","pre-YS.mesoderm","Notochord",
                "Hemogenic endothelial progenitors", "Erythroid","YS.mesoderm") 
# Loop through each cell type and generate a plot
for (cell_type in cell_types) {
  # Get cell names for the current cell type
  cell_names <- rownames(cs8_subset@meta.data[which(cs8_subset$celltype == cell_type), ])
  
  # Generate the plot with highlighted cells for the current cell type
  plot <- DimPlot(cs8_subset, reduction = "spatial", cells.highlight = cell_names, 
                  cols.highlight = "red", cols = "gray",sizes.highlight =0.2) +
    ggtitle(paste("Spatial Plot Highlighting", cell_type))
  
  # Print or save the plot
  print(plot)  # Use print to display each plot in the loop
  
  # Optionally, save each plot as a file
  ggsave(filename = paste0("cross_species_Spatial_Plot_", cell_type, ".pdf"), plot = plot,width = 12, height = 7)
}
