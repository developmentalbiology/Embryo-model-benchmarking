...
def calculate_markers(adata, groupby, min_cells=5, consistency_key=None):
    """
    ...
    adata : AnnData
        Dataset with labels
    groupby : str
        Column in adata.obs to use for grouping (e.g., 'reanno_pred')
    min_cells : int
        Minimum number of cells required for a group to be considered
    consistency_key : str, optional
        Column in adata.obs indicating which cells are consistent
    ...
    # Filter by consistency if available
    if consistency_key is not None and consistency_key in adata.obs:
        consistent_mask = adata.obs[consistency_key].values.astype(bool)
        log_message(f"Using {np.sum(consistent_mask)}/{len(consistent_mask)} consistent cells for marker calculation")
        ...
        adata_copy = adata_copy[consistent_mask]
    else:
        ...
        if 'reanno_pred_lineage' in adata.obs and 'lineage_pred' in adata.obs:
            ...
            log_message(f"Using consistency based on reanno_pred_lineage==lineage_pred: {np.sum(consistent_mask)}/{len(consistent_mask)} cells for marker calculation")
            if np.sum(consistent_mask) > 0:
                adata_copy = adata_copy[consistent_mask]
        else:
            # Fallback to old certainty logic if consistency columns not available
            if 'is_celltype_certain' in adata.obs and groupby == 'reanno_pred':
                consistent_mask = adata.obs['is_celltype_certain'].values.astype(bool)
                log_message(f"Fallback: Using 'is_celltype_certain': {np.sum(consistent_mask)}/{len(consistent_mask)} cells for marker calculation")
                if np.sum(consistent_mask) > 0:
                    adata_copy = adata_copy[consistent_mask]
            elif 'is_lineage_certain' in adata.obs and groupby == 'lineage_pred':
                consistent_mask = adata.obs['is_lineage_certain'].values.astype(bool)
                log_message(f"Fallback: Using 'is_lineage_certain': {np.sum(consistent_mask)}/{len(consistent_mask)} cells for marker calculation")
                if np.sum(consistent_mask) > 0:
                    adata_copy = adata_copy[consistent_mask]
            else:
                log_message(f"No consistency filter found or provided, using all {adata_copy.n_obs} cells for marker calculation")
...
def process_file(query_file, reference_file, output_dir):
    ...
    if 'reanno_pred' in query_adata.obs and 'reanno' in reference_adata.obs:
        ...
        # Get consistent cells mask for query dataset
