...
################################ scgpt #######################################
outputDir <- "./embryo_model_scGPT_scArches_out_csv"

# Get the list of all CSV files in the directory
csv_files <- list.files(path = outputDir, pattern = "_scgpt.csv", full.names = TRUE)

# Read all CSV files into a list
csv_list <- lapply(csv_files, read.csv)

# Optionally, assign names to the list based on the CSV file names
names(csv_list) <- gsub("_scgpt.csv", "", basename(csv_files))
names(csv_list) <- gsub("human_", "", names(csv_list))
...
for (i in 1:length(csv_list)) {
  ...
  obj$scgpt_human_ref_final_anno <- data$scgpt_final_anno_pre
  obj$scgpt_human_ref_final_lineage <- data$scgpt_final_lineage_pre
  
  # Create UMAP plots for the annotations
  p1 <- DimPlot(obj, reduction = "umap", group = "scgpt_human_ref_final_anno", cols = humanref_color_mapping) +
    ggtitle(paste0("scgpt_human_ref_final_anno_", obj_name))
  
  p2 <- DimPlot(obj, reduction = "umap", group = "scgpt_human_ref_final_lineage", cols = lineage_color_mapping) +
    ggtitle(paste0("scgpt_human_ref_final_lineage_", obj_name))
  
  # Save p1 (final_anno) plot as a PDF
  ggsave(
    filename = paste0("scgpt_human_ref_final_anno_", obj_name, ".pdf"),
    plot = p1,
    device = "pdf",
  ...
  ggsave(
    filename = paste0("scgpt_human_ref_final_lineage_", obj_name, ".pdf"),
    plot = p2,
    device = "pdf",
...
################################ scarches #######################################
# Get the list of all CSV files in the directory
csv_files <- list.files(path = outputDir, pattern = "_scArches.csv", full.names = TRUE)

# Read all CSV files into a list
csv_list <- lapply(csv_files, read.csv)

# Optionally, assign names to the list based on the CSV file names
names(csv_list) <- gsub("_scArches.csv", "", basename(csv_files))
names(csv_list) <- gsub("human_", "", names(csv_list))
...
for (i in 1:length(csv_list)) {
  ...
  # Add the querymap annotations to the Seurat object
  obj$scArches_human_ref_final_anno <- data$scArches_final_anno_pre
  obj$scArches_human_ref_final_lineage <- data$scArches_final_lineage_pre
  
  # Create UMAP plots for the annotations
  p1 <- DimPlot(obj, reduction = "umap", group = "scArches_human_ref_final_anno", cols = humanref_color_mapping) +
    ggtitle(paste0("scArches_human_ref_final_anno_", obj_name))
  
  p2 <- DimPlot(obj, reduction = "umap", group = "scArches_human_ref_final_lineage", cols = lineage_color_mapping) +
    ggtitle(paste0("scArches_human_ref_final_lineage_", obj_name))
  
  # Save p1 (final_anno) plot as a PDF
  ggsave(
    filename = paste0("scArches_human_ref_final_anno_", obj_name, ".pdf"),
    plot = p1,
    device = "pdf",
    width = 8,   # Adjust the width as needed
    height = 4   # Adjust the height as needed
  ...
  # Save p2 (final_lineage) plot as a PDF
  ggsave(
    filename = paste0("scArches_human_ref_final_lineage_", obj_name, ".pdf"),
    plot = p2,
    device = "pdf",
    width = 5,   # Adjust the width as needed
    height = 4   # Adjust the height as needed
