...
setwd("D:/xueying-work/afterPHD/Liu-lab/project1-embryo benchmarking/2024April/manuscript/final code/20250225-scPoli_weatherbee_GPU")
...
outputDir <- ".//figures_final_anno"
csv_files_anno <- list.files(path = outputDir, pattern = "_scPoli_query.csv", full.names = TRUE)

outputDir <- ".//figures_final_lineage"
csv_files_lineage <- list.files(path = outputDir, pattern = "_scPoli_query.csv", full.names = TRUE)

# Read all CSV files into a list
csv_list_anno <- lapply(csv_files_anno, read.csv)
csv_list_lineage <- lapply(csv_files_lineage, read.csv)

# Optionally, assign names to the list based on the CSV file names
names(csv_list_anno) <- gsub(".h5ad_scPoli_query.csv", "", basename(csv_files_anno))
names(csv_list_anno) <- gsub("corrected_processed_", "", names(csv_list_anno))

names(csv_list_lineage) <- gsub(".h5ad_scPoli_query.csv", "", basename(csv_files_lineage))
names(csv_list_lineage) <- gsub("corrected_processed_", "", names(csv_list_lineage))

# add querymap results to the rds

attri <- c("final_anno_pred", "final_anno_uncert", "final_lineage_pred", "final_lineage_uncert" )


# Loop through each CSV in csv_list_anno and process the corresponding Seurat object
for (i in 1:length(csv_list_anno)) {
  
  # Extract data from the i-th CSV in csv_list_anno
  data <- csv_list_anno[[i]]
  data_lineage <- csv_list_lineage[[i]]
  
  # Match row names between data_anno and data_lineage
  data_lineage <- data_lineage[match(rownames(data), rownames(data_lineage)), ]
  
  data$final_lineage_pred <- data_lineage$final_lineage_pred
  data$final_lineage_uncert <- data_lineage$final_lineage_uncert
  
  # Extract the corresponding Seurat object from obj_new using the name of the i-th CSV file
  obj_name <- names(csv_list_anno)[i]  # Get the name of the i-th CSV file
  ...
  rownames(data) <- gsub("-1$", "", rownames(data))
  ...
  obj$scPoli_human_ref_final_anno <- data$final_anno_pred
  obj$scPoli_human_ref_final_anno_uncert <- data$final_anno_uncert
  obj$scPoli_human_ref_final_lineage <- data$final_lineage_pred
  obj$scPoli_human_ref_final_lineage_uncert <- data$final_lineage_uncert
  
  # Create UMAP plots for the annotations
  p1 <- DimPlot(obj, reduction = "umap", group = "scPoli_human_ref_final_anno", cols = humanref_color_mapping) +
    ggtitle(paste0("scPoli_human_ref_final_anno_", obj_name))
  
  p2 <- DimPlot(obj, reduction = "umap", group = "scPoli_human_ref_final_lineage", cols = lineage_color_mapping) +
    ggtitle(paste0("scPoli_human_ref_final_lineage_", obj_name))
  
  # Save p1 (final_anno) plot as a PDF
  ggsave(
    filename = paste0("scPoli_human_ref_final_anno_", obj_name, ".pdf"),
    plot = p1,
    device = "pdf",
  ...
  ggsave(
    filename = paste0("scPoli_human_ref_final_lineage_", obj_name, ".pdf"),
    plot = p2,
    device = "pdf",
