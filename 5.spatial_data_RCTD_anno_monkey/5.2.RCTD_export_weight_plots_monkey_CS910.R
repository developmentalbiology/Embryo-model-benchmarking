...
setwd("D:/xueying-work/afterPHD/Liu-lab/project1-embryo benchmarking/2024April/manuscript/NCB_rebuttal_2025_june/code/20250706_monkey_RCTD_reanno")
...
RCTD <- readRDS( "RCTD_cs910_monkey_reanno_20250706.rds")
...
#############################################################################
# define function for generating composite weight plots in one pdf
...
pdf("monkey_reanno_score.pdf", width = 25, height = 10)
...
for (i in 1:length(cell_type_names)) {
  ...
  if (sum(weights[, cell_type]) > 0) {
    ...
    pdf(file = paste0("monkey_CS910_RCTD_score_", cell_type, ".pdf"), width = 25, height = 10)
    print(plot)
    dev.off()
  }
}
