...
# load data
cs8 <- readRDS("D:/xueying-work/afterPHD/Liu-lab/project1-embryo benchmarking/2024April/manuscript/final data/public dataset/CS7_human_embryo.rds")
...
sample_num <- as.numeric(sub("S", "", cs8$sample_final))

col_index <- (sample_num - 1) %% tiles_per_row
row_index <- (sample_num - 1) %/% tiles_per_row

cs8$x_new <- cs8$newx + col_index * tile_width
cs8$y_new <- cs8$newy + row_index * tile_height


colnames(cs8@meta.data)[8] <- "x"
colnames(cs8@meta.data)[9] <- "y"
...
DimPlot(cs8, reduction = "spatial", group.by = "celltype")

#############################################################################
# define plot_puck_wrapper for generating composite weight score plots within one pdf
...
pdf("human_CS7_RCTD_score.pdf", width = 25, height = 10)
...
for (i in 1:length(cell_type_names)) {
  ...
  if (sum(weights[, cell_type]) > 0) {
    ...
    pdf(file = paste0("human_CS7_RCTD_score_", cell_type, ".pdf"), width = 25, height = 10)
    print(plot)
    dev.off()
  }
}
