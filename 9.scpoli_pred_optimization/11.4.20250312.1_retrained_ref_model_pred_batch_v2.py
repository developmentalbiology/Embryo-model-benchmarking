...
high_confidence_indices = np.where(
    (embryomodel.obs["human_ref_final_anno_uncert"] < 0.1) | 
...
high_confidence_labels = embryomodel.obs["human_ref_final_anno_pred"].iloc[high_confidence_indices]

print(high_confidence_cells.X)

# Step 4: Extract high-confidence labels (cell type annotations)
high_confidence_labels = embryomodel.obs["human_ref_final_anno_pred"][high_confidence_indices]

# Step 5: select confident cell types 
desired_cell_types = ["epi_3","Amnion", "Neural tube", "AVE", "VE/YE", "Hypoblast", "DE", "PGC"]

# filter cell with correct annotaions 
high_confidence_indices = high_confidence_cells.obs["human_ref_final_anno_pred"].isin(desired_cell_types)

# Extract high-confidence cells
high_confidence_cells = high_confidence_cells[high_confidence_indices]

# Extract high-confidence labels (cell type annotations)
high_confidence_labels = high_confidence_cells.obs["human_ref_final_anno_pred"][high_confidence_indices]
...
# Train reference model for "final_anno" (only once)
...
reference_model_dir = '/storage/liuxiaodongLab/fanxueying/mayanalysis/scPoli/scpoli_optim/reference_model/'
...
query_adata_extended.obs.rename(columns={'human_ref_final_anno_pred': 'final_anno'}, inplace=True)
...
print(combined_adata.obs['final_anno'])
...
try:
    ...
    print(f"Error while saving the enhanced model: {e}")
...
figures_folder = '/storage/liuxiaodongLab/fanxueying/mayanalysis/scPoli/scpoli_optim/figures_final_anno'
...
for h5ad_file in h5ad_files:
    ...
    try:
        ...
        sc.pl.umap(
            ...
            show=False,
            frameon=False,
            save=f'_{file_name}_scPoli_final_anno.png'
        )
        sc.pl.umap(
            adata_no_prototypes,
            color='orig.ident',
            show=False,
            frameon=False,
            save=f'_{file_name}_scPoli_anno_dataset.png'
        )
        sc.pl.umap(
            adata_no_prototypes,
            ...
            cmap='magma',
            vmax=1,
            save=f'_{file_name}_scPoli_anno_uncert.png'
        )
        ...
        sc.pl.umap(
            ...
            show=False,
            frameon=False,
            save=f'_{file_name}_scPoli_anno_query.png'
        )
