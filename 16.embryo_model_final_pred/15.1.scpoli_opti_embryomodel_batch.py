...
output_folder = '/storage2/liuxiaodongLab/fanxueying/mayanalysis/scPoli/20250801_embryomodel_export_plots/output'
figures_folder = '/storage2/liuxiaodongLab/fanxueying/mayanalysis/scPoli/20250801_embryomodel_export_plots/figures'
...
# Define models paths
lineage_model_dir = '/storage2/liuxiaodongLab/fanxueying/embryo_benchmarking_rebuttal/code/20250729_scpoli_optimization_v3/enhanced_reference_model_lineage_2ndround/'
anno_model_dir = '/storage2/liuxiaodongLab/fanxueying/embryo_benchmarking_rebuttal/code/20250729_scpoli_optimization_v3/enhanced_reference_model_reanno_2ndround/'

log_message("Loading lineage reference model...")
# Load lineage reference model
adata_lineage_path = os.path.join(lineage_model_dir, "adata.h5ad")
...
adata_anno_path = os.path.join(anno_model_dir, "adata.h5ad")
...
# Function to safely convert to array (handles both sparse and dense matrices)
def safe_to_array(matrix):
    """
    Safely convert matrix to numpy array, handling both sparse and dense matrices.
    """
...
def extend_query_data(query_adata, source_var_names):
    ...
    if 'counts' in query_adata.layers:
        ...
        if sp.issparse(query_adata.layers['counts']):
            ...
            log_message("  Counts layer is dense matrix")
...
for idx, h5ad_file in enumerate(h5ad_files):
    ...
    try:
        ...
        # FIXED: Preprocess query dataset if needed - handle both sparse and dense matrices
        if 'counts' not in query_adata.layers:
            log_message("Adding counts layer to query data")
            # Use safe_to_array to handle both sparse and dense matrices
            query_adata.layers["counts"] = safe_to_array(query_adata.X.copy())
        else:
            log_message("Counts layer already exists")
            # Check if counts layer needs conversion
            if sp.issparse(query_adata.layers["counts"]):
                log_message("Counts layer is sparse matrix - keeping as is")
            else:
                log_message("Counts layer is dense matrix - keeping as is")
        ...
        # Set lineage to Unknown
        log_message("Setting lineage to 'Unknown'")
        query_adata_lineage_ext.obs['lineage'] = 'Unknown'
        ...
        log_message(f"Lineage predictions completed with {len(results_lineage['lineage']['preds'])} predictions")
        ...
        adata_latent_lineage.obs['lineage_pred'] = results_lineage['lineage']['preds'].tolist()
        adata_latent_lineage.obs['lineage_uncert'] = results_lineage['lineage']['uncert'].tolist()
        ...
        if "lineage_pred" not in query_adata.obs.columns:
            query_adata.obs["lineage_pred"] = np.nan
        if "lineage_uncert" not in query_adata.obs.columns:
            query_adata.obs["lineage_uncert"] = np.nan
            
        query_adata.obs.loc[matching_indices, "lineage_pred"] = adata_latent_lineage.obs.loc[matching_indices, "lineage_pred"]
        query_adata.obs.loc[matching_indices, "lineage_uncert"] = adata_latent_lineage.obs.loc[matching_indices, "lineage_uncert"]
        ...
        # Set reanno to Unknown
        log_message("Setting reanno to 'Unknown'")
        query_adata_anno_ext.obs['reanno'] = 'Unknown'
        ...
        log_message(f"Annotation predictions completed with {len(results_anno['reanno']['preds'])} predictions")
        ...
        # Add predictions to latent data
        log_message("Adding predictions to latent data")
        adata_latent_anno.obs['reanno_pred'] = results_anno['reanno']['preds'].tolist()
        adata_latent_anno.obs['reanno_uncert'] = results_anno['reanno']['uncert'].tolist()
        ...
        if "reanno_pred" not in query_adata.obs.columns:
            query_adata.obs["reanno_pred"] = np.nan
        if "reanno_uncert" not in query_adata.obs.columns:
            query_adata.obs["reanno_uncert"] = np.nan
            
        query_adata.obs.loc[matching_indices, "reanno_pred"] = adata_latent_anno.obs.loc[matching_indices, "reanno_pred"]
        query_adata.obs.loc[matching_indices, "reanno_uncert"] = adata_latent_anno.obs.loc[matching_indices, "reanno_uncert"]
        ...
        sc.pl.umap(adata_latent_lineage, color=['lineage_pred', 'lineage_uncert'], 
                  save=f'_{file_name}_lineage.pdf', show=False)
        sc.pl.umap(adata_latent_anno, color=['reanno_pred', 'reanno_uncert'], 
