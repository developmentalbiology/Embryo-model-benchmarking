...
def calculate_certainty(adata, uncertainty_threshold=0.2):
    ...
    if 'reanno_pred' in adata.obs and 'reanno_uncert' in adata.obs:
        celltype_certain = adata.obs['reanno_uncert'] < uncertainty_threshold
    ...
    if 'lineage_pred' in adata.obs and 'lineage_uncert' in adata.obs:
        lineage_certain = adata.obs['lineage_uncert'] < uncertainty_threshold
    ...
    if 'reanno_pred' in adata.obs and 'reanno_uncert' in adata.obs:
        per_celltype = {}
        for ct in adata.obs['reanno_pred'].unique():
            mask = adata.obs['reanno_pred'] == ct
            ct_certainty = np.mean(adata.obs.loc[mask, 'reanno_uncert'] < uncertainty_threshold)
            count = np.sum(mask)
    ...
    if 'lineage_pred' in adata.obs and 'lineage_uncert' in adata.obs:
        per_lineage = {}
        for lin in adata.obs['lineage_pred'].unique():
            mask = adata.obs['lineage_pred'] == lin
            lin_certainty = np.mean(adata.obs.loc[mask, 'lineage_uncert'] < uncertainty_threshold)
            count = np.sum(mask)
...
def compare_certainty_distributions(all_files, output_dir, certainty_type='both'):
    ...
    for file_path in all_files:
        ...
        try:
            ...
            if 'reanno_uncert' in adata.obs and (certainty_type == 'both' or certainty_type == 'celltype'):
                for idx, uncert in enumerate(adata.obs['reanno_uncert']):
            ...
            if 'lineage_uncert' in adata.obs and (certainty_type == 'both' or certainty_type == 'lineage'):
                for idx, uncert in enumerate(adata.obs['lineage_uncert']):
                    lineage_data.append({
                        'Model': model_name,
                        'Uncertainty': uncert,
                        'Cell ID': adata.obs.index[idx]
