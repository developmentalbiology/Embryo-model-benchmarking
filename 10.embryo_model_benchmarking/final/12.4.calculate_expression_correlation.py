...
def calculate_matching_label_correlation(query_data, reference_data, 
    ...
    for label in common_labels:
        # Extract cells with this label from query (already filtered for certain cells)
        ...
        if np.sum(query_mask) == 0:
            log_message(f"Skipping {label}: No certain cells in query")
            continue
        ...
        log_message(f"Processing label: {label} - {np.sum(query_mask)} certain query cells, {np.sum(ref_mask)} reference cells")
...
def visualize_label_correlations(metrics, file_name, output_dir, label_type="Cell Type"):
    ...
    if "correlation_df" in metrics:
        ...
        plt.title(f"{label_type} Gene Expression Correlation with Reference\n{file_name}\nUsing {metrics['certain_cells']} certain cells and {metrics['common_genes']} genes")
...
def process_file(query_file, reference_file, output_dir):
    ...
    if 'final_anno_pred' in query_adata.obs:
    ...
    if 'final_lineage_pred' in query_adata.obs:
        ...
        metrics['lineage_correlation'] = calculate_matching_label_correlation(
            query_adata, reference_adata,
            query_label_key='final_lineage_pred',
            ref_label_key='final_lineage',
            certainty_key=certainty_key
        )
    ...
    if len(all_metrics) > 1:
        ...
        for model, metrics in all_metrics.items():
            if 'celltype_correlation' in metrics and 'error' not in metrics['celltype_correlation']:
                ...
                ct_data.append({
                    ...
                    'Certain Cells': correlation.get('certain_cells', 0)
                })
        ...
        if ct_data:
            ...
            for i, row in ct_df.iterrows():
                ax.text(i, row['Pearson r'] + 0.02, f"{row['Certain Cells']} cells", 
